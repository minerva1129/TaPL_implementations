    ...
    | (TyRecord(fS), TyRecord(fT)) ->
        ...
        let commonFields =
          List.map (fun li ->
            ...
            (li, join tySi tyTi)) commonLabels in
...
and meet tyS tyT =
  match (tyS, tyT) with
    ...
    | (TyRecord(fS), TyRecord(fT)) ->
        let labelsS = List.map (fun (li, _) -> li) fS in
        let labelsT = List.map (fun (li, _) -> li) fT in
        let allLabels = List.append labelsS (List.find_all (fun l -> not (List.mem l labelsS)) labelsT) in
        let allFields =
          List.map (fun li ->
            match (List.mem li labelsS, List.mem li labelsT) with
              | (true, true) ->
                  let tySi = List.assoc li fS in
                  let tyTi = List.assoc li fT in
                  (li, meet tySi tyTi)
              | (true, false) ->
                  (li, List.assoc li fS)
              | (false, true) ->
                  (li, List.assoc li fT)) allLabels in
        TyRecord(allFields)
    | _ ->
        raise Not_found
